...
namespace json {
	...
	Node& Builder::Build() {
		if (!Ready()) throw std::logic_error("a closing parenthesis is expected"s);
		return root_;
	}

	bool Builder::Ready() {
		if (nodes_stack_.size()) return false;
		return start_;
	}

	void Builder::ReadyToAdd() {
		if (Ready()) throw std::logic_error("the formation has already been completed"s);
	}
	...
	BuilderBeforeKey Builder::StartDict() {
		...
		if (nodes_stack_.size() == 0)
		{
			...
			if (nodes_stack_.back()->IsArray()) {
				...
				if (!key_received) throw std::logic_error("the dictionary requires a key"s);
				auto& dict = const_cast<Dict&>(nodes_stack_.back()->AsDict());
				dict.insert({ name_, Dict{} });
	...
	Builder& Builder::EndDict() {
		ReadyToAdd();
		if (!nodes_stack_.back()->IsDict()) throw std::logic_error("dictionary closure is not expected"s);
	...
	BuilderAfteKey Builder::Key(const std::string s) {
		ReadyToAdd();
		if (!nodes_stack_.back()->IsDict()) throw std::logic_error("the name is only for the dictionary"s);
		if (key_received) throw std::logic_error("the name has already been received"s);
		name_ = s;
		key_received = true;
		return  *this;
	}
	...
	Builder& Builder::EndArray() {
		ReadyToAdd();
		if (!nodes_stack_.back()->IsArray()) throw std::logic_error("array closure is not expected"s);
		nodes_stack_.pop_back();
		start_ = true;
		return *this;
	}
